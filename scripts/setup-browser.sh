...
# - Google Chrome: the real headed browser the claude-in-chrome MCP attaches
#   to via CDP at localhost:9222 (driven by Claude CLI's `--chrome` flag)
#
# Idempotent: safe to re-run. apt-get install is a no-op if packages already
# present. Most boxes that ran the old host-mode humphrey will already have

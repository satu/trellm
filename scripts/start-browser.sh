...
#
# The Claude CLI's `--chrome` flag attaches to Chrome via CDP at
# localhost:9222 through the Web-Store-installed `claude-in-chrome`
# extension that lives in ~/.chrome-trellm. View/control the browser via
# noVNC at http://<host>:6080/vnc.html or any VNC client on port 5900.
#
# This is the trellm-side counterpart of humphrey's scripts/start-browser.sh.
# We re-use the standard ports (5900, 6080, 9222, display :99) because
# humphrey now runs in its own Docker container with bridge networking and
# no published ports — there's no host-side conflict.
#
...
# claude-in-chrome extension ID (Chrome Web Store). Same ID humphrey uses.
...
start_chrome() {
  ...
  # Make Claude CLI's extension detection see trellm's profile.
  # Claude CLI scans ~/.config/google-chrome/Default/Extensions/ for the
  # `claude-in-chrome` extension; our profile lives at $CHROME_USER_DATA,
  # so symlink it on every start.
  mkdir -p "${HOME}/.config/google-chrome/Default/Extensions"
  ln -sfn "${CHROME_USER_DATA}/Default/Extensions/${EXTENSION_ID}" \
          "${HOME}/.config/google-chrome/Default/Extensions/${EXTENSION_ID}"

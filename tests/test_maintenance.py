...
class TestMaintenanceChromeFlag:
    """M3 — verify run_maintenance + its inner _run_compact propagate the
    `--chrome` flag (so maintenance runs share the headed-Chrome session
    when the project has it enabled)."""

    @pytest.mark.asyncio
    async def test_maintenance_omits_chrome_flag_by_default(self, tmp_path):
        mock_proc = AsyncMock()
        mock_proc.returncode = 0
        mock_proc.communicate = AsyncMock(
            return_value=(
        ...
        with patch(
            ...
            assert "--chrome" not in mock_exec.call_args[0]

    @pytest.mark.asyncio
    async def test_maintenance_appends_chrome_flag_when_enabled(self, tmp_path):
        mock_proc = AsyncMock()
        mock_proc.returncode = 0
        mock_proc.communicate = AsyncMock(
            return_value=(
        ...
        with patch(
            ...
            assert "--chrome" in mock_exec.call_args[0]

    @pytest.mark.asyncio
    async def test_maintenance_compact_path_carries_chrome_flag(self, tmp_path):
        """When maintenance compacts an existing session before running,
        the compact spawn must also carry --chrome — otherwise the compact
        binary tries to attach without the extension and errors out."""
        compact_proc = AsyncMock()
        compact_proc.returncode = 0
        compact_proc.communicate = AsyncMock(
        ...
        with patch(
            ...
            # Both calls (compact then maintenance) must contain --chrome
            assert mock_exec.call_count == 2
            for call in mock_exec.call_args_list:
                assert "--chrome" in call.args, (
                    f"--chrome missing from spawn: {call.args}"
                )
